            ...
            <div v-else class="plist">
              <div class="prow" v-for="p in rows" :key="p.id">
                ...
                <label class="pname">
                  ...
                  <small v-if="p.type === 1" class="range"> {{ p.minValue ?? '-∞' }} ~ {{ p.maxValue ?? '+∞' }} </small>
                </label>
                ...
                <span v-else class="datalist-wrap">
                  ...
                  />
                  <datalist :id="'dl-' + p.id">
                    <option v-for="m in p.mapEntries || []" :key="m.key" :value="m.key">{{ m.key }}</option>
                  </datalist>
                </span>
              </div>
            </div>
...
:deep(.mindtree__links .mindtree__link) {
  stroke: #dbe1ff;
  stroke-width: 2px;
}
