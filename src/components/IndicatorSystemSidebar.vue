...
async function fetchData() {
  ...
  try {
    ...
    const backendSize =
      typeof data.size === 'number' ? data.size : typeof data.pageSize === 'number' ? data.pageSize : size.value

    // 总页数：优先 data.pages；否则用 total/size 计算（至少 1）
    pages.value =
      typeof data.pages === 'number' ? data.pages : Math.max(1, Math.ceil(totalNum / Math.max(1, backendSize)))
...
function requestCreate() {
  emit('request-create-system')
}
function requestEdit(sys: IndicatorSystem) {
  emit('request-edit-system', sys)
}
function confirmDelete(sys: IndicatorSystem) {
  emit('request-delete-system', sys)
}
