...
import { isNull } from '@/utils/common/validate.ts'
...
export const useEcharts = (dom: Ref<HTMLDivElement | HTMLCanvasElement | null>, config?: ConfigProps) => {
  const {
    echartsInitOpts,
    animation,
    animationDuration,
  ...
  const downloadImage = (fileName: string, options?: DataURLOptions) => {
    ...
    const finalFileName = /^[a-z0-9]+$/i.test(fileName.trim()) ? fileName : `${fileName.trim()}.${baseOptions.type}`
